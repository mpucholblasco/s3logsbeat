...
func TestS3ALBLogParseSingleLine(t *testing.T) {
	logs := logHTTP
...
func TestS3ALBLogParserMultipleLinesDifferentTypes(t *testing.T) {
	var b bytes.Buffer
	b.WriteString(logHTTP)
	b.WriteByte('\n')
	b.WriteString(logHTTPS)
	b.WriteByte('\n')
	b.WriteString(logHTTP2)
	b.WriteByte('\n')
	b.WriteString(logWS)
	b.WriteByte('\n')
	b.WriteString(logWSS)
	logs := b.String()
	...
	expected := []common.MapStr{
		common.MapStr{
			...
			"type":                     "https",
			"time":                     time.Date(2016, 8, 10, 23, 39, 43, 65466000, time.UTC),
			"elb":                      "app/my-loadbalancer/50dc6c495c0c9188",
			"client_ip":                "192.168.131.39",
			"client_port":              uint16(2817),
			"target_ip":                "10.0.0.1",
		...
		common.MapStr{
			"type":                     "h2",
			"time":                     time.Date(2016, 8, 10, 00, 10, 33, 145057000, time.UTC),
			"elb":                      "app/my-loadbalancer/50dc6c495c0c9188",
		...
		common.MapStr{
			"type":                     "ws",
			"time":                     time.Date(2016, 8, 10, 00, 32, 8, 923954000, time.UTC),
			"elb":                      "app/my-loadbalancer/50dc6c495c0c9188",
		...
		common.MapStr{
			"type":                     "wss",
			"time":                     time.Date(2016, 8, 10, 00, 42, 46, 423695000, time.UTC),
			"elb":                      "app/my-loadbalancer/50dc6c495c0c9188",
			"client_ip":                "10.0.0.140",
...
func TestS3ALBLogParseStringToKindsWithError(t *testing.T) {
	...
	assert.Nil(t, v)
	assert.EqualError(t, err, "Couldn't convert to unsupported kind (NotExisting)")
}
